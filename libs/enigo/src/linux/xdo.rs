...
impl Default for EnigoXdo {
    ...
    fn default() -> Self {
        ...
        if xdo.is_null() {
            log::warn!("[enigo] Failed to create xdo context, xdo functions will be disabled");
        } else {
            log::info!("[enigo] xdo context created successfully");
        }
        Self {
            xdo,
            delay: DEFAULT_DELAY,
...
impl EnigoXdo {
    ...
    pub fn set_delay(&mut self, delay: u64) {
        ...
        if delay > MAX_DELAY {
            log::warn!(
                "[enigo] delay value {} exceeds maximum {}, clamped",
                delay,
                MAX_DELAY
            );
        }
