...
impl MouseControllable for Enigo {
    ...
    fn mouse_move_relative(&mut self, x: i32, y: i32) {
        ...
        let mut new_x = current_x + x;
        let mut new_y = current_y + y;
        ...
        let margin = 50; // pixels from edge to trigger reset

        // Check if cursor is approaching screen boundaries
        let near_edge = new_x < margin
            || new_x > (display_width as i32 - margin)
            || new_y < margin
            || new_y > (display_height as i32 - margin);
        ...
        new_x = new_x.clamp(0, display_width as i32);
        new_y = new_y.clamp(0, display_height as i32);
