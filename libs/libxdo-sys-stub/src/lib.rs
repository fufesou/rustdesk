//! Dynamic loading wrapper for libxdo-sys.
//!
//! This crate provides the same API as libxdo-sys but uses runtime dynamic loading
//! instead of compile-time linking. This allows the program to run on systems
//! without libxdo installed, while still using libxdo when available.

use libc::{c_char, c_int, c_uint};
use libloading::{Library, Symbol};
use std::sync::OnceLock;

// Re-export x11 types for compatibility
pub use x11::xlib::{Display, Screen, Window};

/// Opaque struct representing an xdo context
#[repr(C)]
pub struct xdo_t {
    _private: [u8; 0],
}
...
// CURRENTWINDOW constant used by libxdo
...
type FnXdoNewWithOpenedDisplay = unsafe extern "C" fn(*mut Display, *const c_char, c_int) -> *mut xdo_t;
type FnXdoFree = unsafe extern "C" fn(*mut xdo_t);
type FnXdoSendKeysequenceWindow = unsafe extern "C" fn(*const xdo_t, Window, *const c_char, useconds_t) -> c_int;
type FnXdoSendKeysequenceWindowDown = unsafe extern "C" fn(*const xdo_t, Window, *const c_char, useconds_t) -> c_int;
type FnXdoSendKeysequenceWindowUp = unsafe extern "C" fn(*const xdo_t, Window, *const c_char, useconds_t) -> c_int;
type FnXdoEnterTextWindow = unsafe extern "C" fn(*const xdo_t, Window, *const c_char, useconds_t) -> c_int;
...
type FnXdoMoveMouseRelativeToWindow = unsafe extern "C" fn(*const xdo_t, Window, c_int, c_int) -> c_int;
type FnXdoGetMouseLocation = unsafe extern "C" fn(*const xdo_t, *mut c_int, *mut c_int, *mut c_int) -> c_int;
type FnXdoGetMouseLocation2 = unsafe extern "C" fn(*const xdo_t, *mut c_int, *mut c_int, *mut c_int, *mut Window) -> c_int;
...
type FnXdoGetWindowLocation = unsafe extern "C" fn(*const xdo_t, Window, *mut c_int, *mut c_int, *mut *mut Screen) -> c_int;
type FnXdoGetWindowSize = unsafe extern "C" fn(*const xdo_t, Window, *mut c_uint, *mut c_uint) -> c_int;
...
type FnXdoSetWindowClass = unsafe extern "C" fn(*const xdo_t, Window, *const c_char, *const c_char) -> c_int;
type FnXdoSearchWindows = unsafe extern "C" fn(*const xdo_t, *const xdo_search_t, *mut *mut Window, *mut c_uint) -> c_int;
...
impl XdoLib {
    fn load() -> Option<Self> {
        unsafe {
            ...
            let xdo_send_keysequence_window: FnXdoSendKeysequenceWindow = *lib.get(b"xdo_send_keysequence_window").ok()?;

            // Optional functions (best effort)
            let xdo_new_with_opened_display = lib.get(b"xdo_new_with_opened_display").ok().map(|s: Symbol<FnXdoNewWithOpenedDisplay>| *s);
            let xdo_send_keysequence_window_down = lib.get(b"xdo_send_keysequence_window_down").ok().map(|s: Symbol<FnXdoSendKeysequenceWindowDown>| *s);
            let xdo_send_keysequence_window_up = lib.get(b"xdo_send_keysequence_window_up").ok().map(|s: Symbol<FnXdoSendKeysequenceWindowUp>| *s);
            let xdo_enter_text_window = lib.get(b"xdo_enter_text_window").ok().map(|s: Symbol<FnXdoEnterTextWindow>| *s);
            let xdo_click_window = lib.get(b"xdo_click_window").ok().map(|s: Symbol<FnXdoClickWindow>| *s);
            let xdo_mouse_down = lib.get(b"xdo_mouse_down").ok().map(|s: Symbol<FnXdoMouseDown>| *s);
            let xdo_mouse_up = lib.get(b"xdo_mouse_up").ok().map(|s: Symbol<FnXdoMouseUp>| *s);
            let xdo_move_mouse = lib.get(b"xdo_move_mouse").ok().map(|s: Symbol<FnXdoMoveMouse>| *s);
            let xdo_move_mouse_relative = lib.get(b"xdo_move_mouse_relative").ok().map(|s: Symbol<FnXdoMoveMouseRelative>| *s);
            let xdo_move_mouse_relative_to_window = lib.get(b"xdo_move_mouse_relative_to_window").ok().map(|s: Symbol<FnXdoMoveMouseRelativeToWindow>| *s);
            let xdo_get_mouse_location = lib.get(b"xdo_get_mouse_location").ok().map(|s: Symbol<FnXdoGetMouseLocation>| *s);
            let xdo_get_mouse_location2 = lib.get(b"xdo_get_mouse_location2").ok().map(|s: Symbol<FnXdoGetMouseLocation2>| *s);
            let xdo_get_active_window = lib.get(b"xdo_get_active_window").ok().map(|s: Symbol<FnXdoGetActiveWindow>| *s);
            let xdo_get_focused_window = lib.get(b"xdo_get_focused_window").ok().map(|s: Symbol<FnXdoGetFocusedWindow>| *s);
            let xdo_get_focused_window_sane = lib.get(b"xdo_get_focused_window_sane").ok().map(|s: Symbol<FnXdoGetFocusedWindowSane>| *s);
            let xdo_get_window_location = lib.get(b"xdo_get_window_location").ok().map(|s: Symbol<FnXdoGetWindowLocation>| *s);
            let xdo_get_window_size = lib.get(b"xdo_get_window_size").ok().map(|s: Symbol<FnXdoGetWindowSize>| *s);
            let xdo_get_input_state = lib.get(b"xdo_get_input_state").ok().map(|s: Symbol<FnXdoGetInputState>| *s);
            let xdo_activate_window = lib.get(b"xdo_activate_window").ok().map(|s: Symbol<FnXdoActivateWindow>| *s);
            let xdo_wait_for_mouse_move_from = lib.get(b"xdo_wait_for_mouse_move_from").ok().map(|s: Symbol<FnXdoWaitForMouseMoveFrom>| *s);
            let xdo_wait_for_mouse_move_to = lib.get(b"xdo_wait_for_mouse_move_to").ok().map(|s: Symbol<FnXdoWaitForMouseMoveTo>| *s);
            let xdo_set_window_class = lib.get(b"xdo_set_window_class").ok().map(|s: Symbol<FnXdoSetWindowClass>| *s);
            let xdo_search_windows = lib.get(b"xdo_search_windows").ok().map(|s: Symbol<FnXdoSearchWindows>| *s);
...
fn get_lib() -> Option<&'static XdoLib> {
    XDO_LIB.get_or_init(|| XdoLib::load()).as_ref()
}

// ============================================================================
// Public API - forwards to real libxdo if available, otherwise returns defaults
// ============================================================================
...
pub unsafe extern "C" fn xdo_new_with_opened_display(
    ...
    get_lib()
        ...
        .map_or(std::ptr::null_mut(), |f| f(xdpy, display, close_display_when_freed))
}
...
pub unsafe extern "C" fn xdo_send_keysequence_window(
    ...
    get_lib().map_or(1, |lib| (lib.xdo_send_keysequence_window)(xdo, window, keysequence, delay))
}
...
#[no_mangle]
pub unsafe extern "C" fn xdo_click_window(xdo: *const xdo_t, window: Window, button: c_int) -> c_int {
    get_lib()
...
#[no_mangle]
pub unsafe extern "C" fn xdo_move_mouse(xdo: *const xdo_t, x: c_int, y: c_int, screen: c_int) -> c_int {
    get_lib()
...
#[no_mangle]
pub unsafe extern "C" fn xdo_get_active_window(xdo: *const xdo_t, window_ret: *mut Window) -> c_int {
    get_lib()
...
#[no_mangle]
pub unsafe extern "C" fn xdo_get_focused_window(xdo: *const xdo_t, window_ret: *mut Window) -> c_int {
    get_lib()
...
#[no_mangle]
pub unsafe extern "C" fn xdo_get_focused_window_sane(xdo: *const xdo_t, window_ret: *mut Window) -> c_int {
    get_lib()
...
#[no_mangle]
pub unsafe extern "C" fn xdo_wait_for_mouse_move_from(xdo: *const xdo_t, origin_x: c_int, origin_y: c_int) -> c_int {
    get_lib()
...
#[no_mangle]
pub unsafe extern "C" fn xdo_wait_for_mouse_move_to(xdo: *const xdo_t, dest_x: c_int, dest_y: c_int) -> c_int {
    get_lib()
