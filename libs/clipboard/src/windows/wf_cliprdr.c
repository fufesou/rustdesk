...
static size_t wf_cliprdr_bounded_strlen(const char *value, size_t max_len)
{
	size_t len = 0;
										  ...
										  const CLIPRDR_FORMAT_LIST *formatList)
{
	...
	for (i = 0; i < formatList->numFormats; i++)
	{
		...
		{
			size_t name_len =
				wf_cliprdr_bounded_strlen(format->formatName,
				                          WF_CLIPRDR_MAX_FORMAT_NAME_BYTES);
			int size;

			if (name_len > WF_CLIPRDR_MAX_FORMAT_NAME_BYTES)
			{
			...
			if (mapping->name)
			{
				MultiByteToWideChar(CP_UTF8, 0, format->formatName, (int)name_len,
				                    mapping->name, size);
