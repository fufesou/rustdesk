...
pub fn set_expected_service_peer_uid(uid: Option<u32>) {
    EXPECTED_SERVICE_PEER_UID.store(uid.unwrap_or(0), Ordering::Relaxed);
}
...
pub async fn connect_service(ms_timeout: u64) -> ResultType<ConnectionTmpl<ConnClient>> {
    ...
    for postfix in service_ipc_postfix_candidates() {
    ...
    for postfix in ready_candidates.into_iter() {
        let elapsed_ms = start.elapsed().as_millis() as u64;
        let remaining_ms = ms_timeout.saturating_sub(elapsed_ms);
        if remaining_ms == 0 {
            break;
        }
        let per_attempt_timeout = remaining_ms;
