...
/// activation/deactivation markers for the Rust rdev grab loop (ESC behavior).
///
...
pub fn session_send_mouse(session_id: SessionID, msg: String) {
    if let Ok(m) = serde_json::from_str::<HashMap<String, String>>(&msg) {
        ...
        // This is used by the Rust rdev grab loop to decide whether ESC should exit relative mouse mode.
        // See doc comment above for the message contract.
        if let Some(v) = m.get("relative_mouse_mode") {
            let active = matches!(v.as_str(), "1" | "Y" | "on");
            crate::keyboard::set_relative_mouse_mode_active(&session_id, active);

            // Disable marker is local-only and must never be forwarded.
            if !active {
                return;
            }
            ...
            if m.get("type").map(|t| t.as_str()) != Some("move_relative") {
                return;
            }
            let x_marker = m
                .get("x")
