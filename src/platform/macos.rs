...
fn copy_dmg_to_update_temp_file(
    ...
    if let Some(expected_size) = expected_size {
        let actual_size = source_file.metadata()?.len();
        if actual_size != expected_size {
            bail!(
                "DMG size mismatch for {}: expected {}, got {}",
                dmg_path,
...
mod tests {
    ...
    #[test]
    fn test_copy_dmg_to_update_temp_file_enforces_expected_size() {
        let mut source: &[u8] = b"rustdesk";
        let mut target = Vec::new();

        copy_dmg_with_expected_size(&mut source, &mut target, Some(8), "test.dmg").unwrap();

        assert_eq!(target, b"rustdesk");
    }
    ...
    #[test]
    fn test_verified_dmg_mount_args_are_read_only() {
        assert!(
            hdiutil_attach_args("/tmp/RustDesk.dmg", "/Volumes/RustDesk", true)
                .contains(&"-readonly".to_owned())
        );
    }
}
