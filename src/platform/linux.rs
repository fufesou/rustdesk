...
pub fn set_cursor_pos(x: i32, y: i32) -> bool {
    ...
    XDO.with(|xdo| {
        if let Ok(xdo) = xdo.try_borrow_mut() {
            ...
            unsafe {
                res = xdo_move_mouse(*xdo, x, y, 0) == 0;
            }
        }
    });
    res
...
/// Show or hide cursor - Linux implementation is a no-op.
///
/// On Linux/X11, cursor hiding requires XDefineCursor with an invisible cursor
/// on a specific window, which requires a window handle not available here.
///
/// On Wayland, cursor hiding is managed by the compositor and requires
/// protocol-specific handling.
///
/// For relative mouse mode on Linux, the Flutter side handles cursor hiding
/// by setting SystemMouseCursors.none on the widget, which works within the
/// Flutter window context.
///
/// Returns 0 (no-op).
pub fn show_cursor(_show: bool) -> i32 {
    // No-op on Linux. See doc comment for details.
    log::debug!("show_cursor called on Linux (no-op): show={}", _show);
    0
}
...
pub fn clip_cursor(_rect: Option<(i32, i32, i32, i32)>) -> bool {
    // No-op on Linux. See doc comment for details.
    log::debug!("clip_cursor called on Linux (no-op): rect={:?}", _rect);
    true
}
