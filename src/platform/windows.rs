...
#[inline]
pub fn remove_custom_client_staging_dir(staging_dir: &Path) -> ResultType<()> {
...
pub fn prepare_custom_client_update() -> ResultType<bool> {
    ...
    if custom_client_staging_dir.exists() {
        ...
        if metadata.is_file() {
            ...
            // fs::copy succeeded above, so the file necessarily existed.
            // If is_custom_client() is still false, the file contents were invalid.
    ...
    // NOTE: staging dir cleanup is intentionally skipped here on success.
    // `remove_custom_client_staging_dir` is currently a no-op to avoid TOCTOU risks.
    // See its doc comment for rationale and future work.
    allow_err!(remove_custom_client_staging_dir(&custom_client_staging_dir));
...
{sleep}
    ...
    let mut _restore_session_guard = crate::common::SimpleCallOnReturn {
        ...
        f: Box::new(move || {
            ...
            if main_window_sessions.is_empty() {
                ...
                for &s in main_window_sessions.iter() {
                    if s != 0 {
                        if is_root {
    ...
    // Always restore tray/main window processes after run_cmds, even on failure.
    // The batch script executes `taskkill` early, so processes are already killed
    // regardless of whether later commands (reg, copy, etc.) succeed or fail.
    // Not restoring them would leave the user without tray and main window.
    _restore_session_guard.b = true;
    let update_result = run_cmds(cmds, debug, "update");
    if let Err(ref e) = update_result {
        log::error!("Update commands failed: {}", e);
    }
...
pub fn handle_custom_client_staging_dir_before_update(
    custom_client_staging_dir: &Path,
) -> ResultType<()> {
    ...
    // Note: `remove_custom_client_staging_dir` is currently a no-op to avoid TOCTOU risks
    // in public directories. See its doc comment for rationale.
    if custom_client_staging_dir.exists() {
        log::debug!(
    ...
    if src_path.exists() {
        ...
        if metadata.is_file() {
            // create_dir_all is idempotent — no need to check exists() first,
            // which would introduce a TOCTOU window in this public directory.
...
pub fn update_to(file: &str) -> ResultType<()> {
    if file.ends_with(".exe") {
        let custom_client_staging_dir = if crate::is_custom_client() {
            let dir = get_custom_client_staging_dir();
            handle_custom_client_staging_dir_before_update(&dir)?;
            Some(dir)
        } else {
            None
        };
        if !run_uac(file, "--update")? {
            ...
            if let Some(dir) = custom_client_staging_dir {
                allow_err!(remove_custom_client_staging_dir(&dir));
            }
            bail!(
    ...
    for entry in entries {
        if let Ok(entry) = entry {
            ...
            if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                ...
                // Match files like rustdesk-*.msi, rustdesk-*.exe, or <app_name>-*.msi, <app_name>-*.exe
                // Note: "rustdesk-" prefix is intentionally kept to also clean up default-branded
                // update files that may remain after switching to a custom client.
                let is_update_file = (file_name_lower.starts_with("rustdesk-")
                    || file_name_lower.starts_with(&format!("{}-", app_name)))
                    && (file_name_lower.ends_with(".msi") || file_name_lower.ends_with(".exe"));
                if is_update_file {
