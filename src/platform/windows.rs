...
pub fn show_cursor(show: bool) -> i32 {
    unsafe {
        ...
        let probe = ShowCursor(TRUE);
        let current = ShowCursor(FALSE); // restore: current = probe - 1, but ShowCursor returns new value
        // After probe: we called TRUE then FALSE, net effect is 0.
        // 'probe' is count after +1, 'current' is count after -1 (i.e., original count).
