on run {user, cur_pid, source_path, expected_sha256}
  ...
  set write_new_plists to quoted form of service_executable & " --write-plists;"
  ...
  set check_service to "service_info=$(launchctl print system/" & daemon_label & " 2>/dev/null || true); printf '%s\n' \"$service_info\" | grep -E '^[[:space:]]*state = running[[:space:]]*$' >/dev/null && [ -S " & quoted form of daemon_socket & " ]"
