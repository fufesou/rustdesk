...
fn remove_owned_stale_path(stale_path: Option<(PathBuf, bool)>) {
    let Some((path, true)) = stale_path else {
        return;
    };
    if path.exists() {
...
fn download_file_(
    ...
    let mut stale_path = None;
    {
        ...
        if let Some(downloader) = downloaders.get(&id) {
            ...
            stale_path = Some((downloader.path.clone(), downloader.path_owned));
            downloaders.remove(&id);
        }
    }
    ...
    let mut stale_path_after_check = None;
    {
        ...
        if let Some(existing) = downloaders.get(&id) {
            ...
            stale_path_after_check = Some((existing.path.clone(), existing.path_owned));
            downloaders.remove(&id);
        }
        downloaders.insert(id.clone(), downloader);
    }
    remove_owned_stale_path(stale_path_after_check);
...
pub fn complete_existing_file(
    ...
    let mut stale_path = None;
    {
        ...
        if let Some(existing) = downloaders.get(&id) {
            ...
            stale_path = Some((existing.path.clone(), existing.path_owned));
        }
        downloaders.insert(id.clone(), downloader);
    }
    if let Some((stale_path, true)) = stale_path {
        if stale_path != final_path {
            remove_owned_stale_path(Some((stale_path, true)));
        }
    }
...
async fn do_download(
    ...
    tokio::select! {
        ...
        created_client = create_http_client_async_with_url(&url) => {
