...
const PENDING_DOWNLOAD_SIZE: u64 = 0;
...
fn resolve_download_size(
    expected_size: Option<u64>,
    response_size: Option<u64>,
    url: &str,
) -> ResultType<u64> {
    if let (Some(expected_size), Some(response_size)) = (expected_size, response_size) {
        ensure_downloaded_size_matches(response_size, expected_size, url)?;
    }
...
pub fn download_file(
    ...
    let Some(path) = path else {
        bail!("A destination path is required for file downloads");
    };
    download_file_(
        url,
        path,
        auto_del_dur,
        None,
        Box::new(|path| Ok(path.to_path_buf())),
    )
}
...
fn download_file_(
    ...
    let downloader = Downloader {
        ...
        path: path.clone(),
        path_owned: false,
        total_size: expected_size.unwrap_or(PENDING_DOWNLOAD_SIZE),
        auto_del_dur,
        downloaded_size: 0,
        error: None,
        tx_cancel: tx,
...
async fn do_download(
    ...
    let total_size = resolve_download_size(expected_size, response.content_length(), &url)?;
...
mod tests {
    ...
    #[test]
    fn legacy_download_entry_point_remains_available() {
        let _download: fn(String, Option<PathBuf>, Option<Duration>) -> ResultType<String> =
            download_file;
    }

    #[test]
    fn unsigned_download_uses_response_content_length() {
        assert_eq!(
            resolve_download_size(None, Some(8), "https://example.com/file").unwrap(),
            8
        );
        assert!(resolve_download_size(None, None, "https://example.com/file").is_err());
    }

    #[test]
    fn signed_download_rejects_response_size_mismatch() {
        assert!(resolve_download_size(Some(8), Some(7), "https://example.com/file").is_err());
    }
