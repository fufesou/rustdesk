            ...
            Ok(is_all_downloaded) => {
                let (downloaded_size, total_size) =
                    with_downloader_if_token_matches(&id2, token, |downloader| {
                        (downloader.downloaded_size, downloader.total_size.unwrap_or(0))
                    })
                    .unwrap_or((0, 0));
                log::info!(
                    "Download {} end, {}/{}, {:.2} %",
...
mod tests {
    ...
    fn download_file_with_finish_handler_does_not_reuse_existing_downloader() {
        ...
        let result =
            download_file_with_finish_handler(id.clone(), Some(path), None, Some(8), |_| {
                unreachable!()
            });
    ...
    fn stale_worker_state_updates_do_not_mutate_replaced_downloader() {
        ...
        assert!(!finish_download(&id, stale_token, Some(PathBuf::from("stale.exe"))));
