...
pub fn global_clean() {
    // Restore cursor visibility and release cursor clip on exit.
    // This is a safety net for relative mouse mode - ensures cursor is usable
    // even if the application exits unexpectedly without proper cleanup.
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        // Best-effort cursor restoration - ignore errors
        let _ = crate::show_cursor(true);
        let _ = crate::clip_cursor(None);
    }
}
