...
fn input_text_via_clipboard_server(en: &mut Enigo, text: &str) {
    ...
    en.key_down(Key::Raw(XKB_KEY_INSERT)).ok();
...
fn translate_keyboard_mode(evt: &KeyEvent) {
    match &evt.union {
        Some(key_event::Union::Seq(seq)) => {
            ...
            // 1. For uinput mode: the uinput service runs as a thread in the --service (root)
            //    process, which lacks user session environment variables (WAYLAND_DISPLAY,
            //    XDG_RUNTIME_DIR). Clipboard operations (arboard::Clipboard::new()) would fail
            //    there. The --server process runs as the logged-in user with the full session
            //    environment, so clipboard operations must happen here.
            ...
            if !crate::platform::linux::is_x11() {
                ...
                if is_hotkey_modifier_pressed(&mut en) {
                    ...
                    for chr in seq.chars() {
                        if !crate::server::uinput::service::is_ascii_printable(chr) {
                            log::warn!(
        ...
        _ => {
            log::debug!("Unreachable. Unexpected key event (mode={:?}, down={:?})", &evt.mode, &evt.down);
        }
    }
}
