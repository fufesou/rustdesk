...
pub(super) async fn check_init() -> ResultType<()> {
    log::debug!("================================= Wayland check_init entered");
    if !is_x11() {
        let mut minx = 0;
        let mut maxx = 0;
        let mut miny = 0;
        ...
        if CAP_DISPLAY_INFO.read().unwrap().is_empty() {
            ...
            if lock.is_empty() {
                ...
                log::debug!("================================ Wayland check_init: Calling Display::all()");
                let all = Display::all()?;
                log::debug!("=========================== Wayland check_init: Display::all() returned {} displays", all.len());
                *PIPEWIRE_INITIALIZED.write().unwrap() = true;
                ...
                for (idx, display) in all.into_iter().enumerate() {
                    log::debug!("=========================== Wayland check_init: Creating capturer for display index {}", idx);
                    let capturer =
                        Box::into_raw(Box::new(Capturer::new(display).with_context(|| {
                            format!("Failed to create capturer for display {}", idx)
                        })?));
                    log::debug!("========================= Wayland check_init: Successfully created capturer for display index {}", idx);
                ...
                log::debug!("========================== Wayland check_init: Finished creating all capturers");
            }
        }
