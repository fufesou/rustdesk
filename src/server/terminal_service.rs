...
use super::terminal_helper::{
    ...
    OwnedHandle, SendableHandle, WinCloseHandle, WinTerminateProcess, WinWaitForSingleObject,
    HELPER_GRACEFUL_EXIT_TIMEOUT_MS, MSG_TYPE_DATA, PIPE_CONNECTION_TIMEOUT_MS, WIN_WAIT_OBJECT_0,
};
...
impl TerminalSession {
    ...
    fn stop(&mut self) {
        ...
        // First, give it a chance to exit gracefully by closing pipes (already done above
        // by dropping input_tx). Wait with WaitForSingleObject, then force terminate if timeout.
        #[cfg(target_os = "windows")]
        if let Some(helper_handle) = self.helper_process_handle.take() {
            let handle = helper_handle.as_raw();
            log::debug!("Waiting for helper process to exit gracefully...");

            // Wait for graceful exit using WaitForSingleObject (more efficient than sleep + poll)
            // The helper should detect pipe EOF and exit on its own
            let wait_result =
                unsafe { WinWaitForSingleObject(handle, HELPER_GRACEFUL_EXIT_TIMEOUT_MS as u32) };

            if wait_result == WIN_WAIT_OBJECT_0 {
                log::debug!("Helper process exited gracefully");
            } else {
                // Timeout or error - force terminate
                log::warn!("Helper process did not exit in time, forcing termination");
                unsafe {
...
impl TerminalServiceProxy {
    ...
    fn handle_open_with_helper(
        ...
        // Wrap handle in OwnedHandle for RAII cleanup on error
        let helper_process_handle = OwnedHandle::new(helper_process_info.handle);
        let helper_pid = helper_process_info.pid;

        // Wait for helper to connect to pipes
        // OwnedHandle ensures automatic cleanup on error
        ...
        if !is_helper_process_running(helper_process_handle.as_raw()) {
            return Err(anyhow!(
        ...
        // Transfer helper process handle ownership to session
        // (extract from OwnedHandle so it won't be closed when we leave this scope)
        let helper_raw_handle = helper_process_handle.into_raw();
