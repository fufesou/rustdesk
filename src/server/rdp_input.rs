...
use super::input_service::{is_ascii_printable, set_clipboard_for_paste_sync};
...
pub mod client {
    ...
    impl KeyboardControllable for RdpInputKeyboard {
        ...
        fn key_up(&mut self, key: Key) {
            // Update modifier state before send — on release, always mark as released
            // to avoid stuck-modifier state even if the release event fails.
