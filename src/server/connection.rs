...
impl Connection {
                    ...
                    if handle_fa {
                        ...
                        match fa.union {
                            ...
                            Some(file_action::Union::Cancel(c)) => {
                                ...
                                let _ = self.pending_read_validations.remove(&c.id);
    ...
    /// Create a new read job and start processing it (Connection-side, non-Windows).
    ///
    /// NOTE: This is the Connection-side read job creation for non-Windows platforms.
    /// On Windows, read jobs are created in CM via `start_read_job()` in `src/ui_cm_interface.rs`.
    /// Both implementations use `TransferJob::new_read()` with similar parameters.
    /// When modifying job creation logic, ensure both paths stay in sync.
