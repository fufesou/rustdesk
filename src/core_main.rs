            ...
            if args[0] == "--update" {
                match parse_macos_update_args(&args) {
                    ...
                    Ok(MacosUpdateArgs::LegacyDmg { dmg_path }) => {
                        let warning = "legacy offline update: signed metadata not found; release hash was not verified";
                        eprintln!("{}", warning);
                        log::warn!("{}", warning);
                        println!("Updating from DMG: {}", dmg_path);
                        if let Some(code) = macos_update_exit_code(
                            platform::update_from_dmg(&dmg_path),
                            "Failed to start update from DMG",
                        ) {
                            std::process::exit(code);
                        }
...
mod tests {
    ...
    #[test]
    fn macos_update_args_parse_verified_legacy_and_app() {
        assert_eq!(
    ...
    #[test]
    fn macos_update_args_accept_dmg_extension_case_insensitively() {
        assert!(matches!(
            parse_macos_update_args(&args(&[
                "--update",
                "/tmp/RustDesk.DMG",
    ...
    fn macos_update_args_reject_invalid_update_target() {
        let error =
            parse_macos_update_args(&args(&["--update", "/tmp/RustDesk.zip"])).unwrap_err();
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn macos_update_launch_failure_requests_exit_code() {
        assert_eq!(
            macos_update_exit_code::<&str>(Err("failed to start"), "failed"),
            Some(1)
        );
        assert_eq!(macos_update_exit_code::<&str>(Ok(()), "failed"), None);
    }
}
