...
mod tests {
    ...
    fn fixed_test_release_download_url_is_accepted() {
        let url = "https://github.com/fufesou/rustdesk/releases/download/fix-update-metadata/rustdesk.exe";
        assert_eq!(
    ...
    #[test]
    fn verified_update_artifact_cache_keeps_only_last_verified_url() {
        let artifact = |url: &str| VerifiedUpdateArtifact {
        ...
        let first_url =
            "https://github.com/fufesou/rustdesk/releases/download/fix-update-metadata/first.exe";
        let second_url =
            "https://github.com/fufesou/rustdesk/releases/download/fix-update-metadata/second.exe";
        *VERIFIED_UPDATE_ARTIFACT.lock().unwrap() = Some(artifact(first_url));
        *VERIFIED_UPDATE_ARTIFACT.lock().unwrap() = Some(artifact(second_url));

        assert!(verified_update_artifact_for_download_url(first_url).is_err());
        assert!(verified_update_artifact_for_download_url(second_url).is_ok());
    }
}
