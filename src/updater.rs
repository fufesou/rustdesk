...
#[cfg(target_os = "macos")]
struct MacUpdateLock(std::path::PathBuf);
...
fn acquire_mac_update_lock() -> ResultType<MacUpdateLock> {
    ...
    for _ in 0..2 {
        ...
        match file.open(&path) {
            Ok(mut handle) => {
                ...
                handle.sync_all()?;
                return Ok(MacUpdateLock(path));
            }
            Err(err) if err.kind() == std::io::ErrorKind::AlreadyExists => {
                let active = std::fs::read_to_string(&path)
                    .ok()
                    .and_then(|pid| pid.trim().parse::<i32>().ok())
                    .is_some_and(|pid| unsafe { hbb_common::libc::kill(pid, 0) == 0 });
                if active {
    ...
    bail!("[root-update] unable to acquire update lock")
}
...
pub fn has_no_active_conns_ipc() -> bool {
    ...
    rt.block_on(async {
        ...
        let output = match std::process::Command::new("/usr/bin/who").output() {
            Ok(o) => o,
            Err(_) => return false,
        };
        ...
        for line in who_output.lines() {
            ...
            if let Ok(id_output) = std::process::Command::new("/usr/bin/id")
                .arg("-u")
                .arg(username)
                .output()
            {
        ...
        if uids.is_empty() {
            ...
            if let Ok(uid) = uid_str.trim().parse::<u32>() {
                // UID 0 on /dev/console is the macOS login window, not an
                // interactive RustDesk user session.  There is no gui/0
                // server IPC endpoint to query in that state.
                if uid == 0 {
                    return true;
                }
                uids.insert(uid);
...
pub fn start_auto_update_macos() {
    let spawn_result = std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            std::thread::sleep(INITIAL_CHECK_DELAY);
            // Consume the failure marker so it does not suppress the first check.
            // The normal retry interval is applied after this check if it fails.
            if std::path::Path::new("/var/root/.rustdeskupdate_failed").exists() {
                let _ = std::fs::remove_file("/var/root/.rustdeskupdate_failed");
                log::info!("[root-update] Previous update attempt failed; retrying now.");
            }
    ...
    if let Ok(entries) = std::fs::read_dir("/tmp") {
        for entry in entries.flatten() {
                ...
                || name_str.starts_with(".rustdeskdownload-")
            {
                ...
                if metadata.file_type().is_dir() && metadata.uid() == 0 && mode == 0o700 {
