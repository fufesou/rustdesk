...
pub(crate) fn current_update_arch() -> &'static str {
    ...
    #[cfg(all(any(target_os = "windows", target_os = "macos"), feature = "flutter"))]
    {
        "x86_64"
    }
    #[cfg(all(any(target_os = "windows", target_os = "macos"), target_arch = "aarch64"))]
    {
        "aarch64"
    }
    #[cfg(not(any(target_os = "windows", target_os = "macos")))]
...
pub(crate) fn current_update_format(update_msi: bool) -> &'static str {
    #[cfg(any(not(target_os = "windows"), all(target_os = "windows", not(feature = "flutter"))))]
    let _ = update_msi;
...
pub(crate) fn install_verified_download(temp_path: &Path, final_path: &Path) -> ResultType<()> {
    ...
    if let Some(backup_path) = backup_path {
        if let Err(e) = std::fs::remove_file(&backup_path) {
            log::warn!("Failed to remove update backup {}: {}", backup_path.display(), e);
        }
    }
    Ok(())
}
