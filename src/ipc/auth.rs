...
impl ConnectionTmpl<parity_tokio_ipc::Connection> {
    ...
    pub(crate) fn portable_service_authorization_status_for_session(
        ...
        // Portable-service policy is intentionally stricter than service-scoped IPC policy:
        // only same-session and non-SYSTEM peers are accepted.
        ...
        let same_session =
            expected_active_session_id.is_some() && peer_session_id == expected_active_session_id;
        let not_system = matches!(peer_is_system, Some(false));
        (
            same_session && not_system,
            peer_pid,
            peer_session_id,
            peer_is_system,
        )
...
mod tests {
    ...
    #[test]
    #[cfg(windows)]
    fn test_portable_service_not_system_policy_fail_closed_on_unknown() {
        let not_system = matches!(None::<bool>, Some(false));
        assert!(
            !not_system,
            "portable service auth must reject unknown SYSTEM status"
        );
    }
