...
class GenerateUpdateMetadataTest(unittest.TestCase):
    ...
    def test_sign_rejects_stable_tag_version_mismatch(self):
        ...
        result = self.run_script(
            ...
            "RUSTDESK_UPDATE_ED25519_SEED",
            "--metadata-out",
            str(self.root / "metadata-mismatch-version.json"),
            "--signature-out",
            str(self.root / "signature-mismatch-version.json"),
            env={"RUSTDESK_UPDATE_ED25519_SEED": self.seed_b64},
            check=False,
        )
        self.assertNotEqual(result.returncode, 0)
    ...
    def test_check_version_accepts_matching_stable_tag_only(self):
        ...
        for release_id in ["v1.4.7-1", "1.4.7-1"]:
            result = self.run_script(
                "check-version",
                "--release-id",
                release_id,
            ...
            self.assertNotEqual(result.returncode, 0, release_id)
            self.assertIn("not supported", result.stderr.lower())
