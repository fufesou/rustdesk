...
class GenerateUpdateMetadataTest(unittest.TestCase):
    ...
    def test_sign_sorts_fragments_and_outputs_base64_signature(self):
        ...
        artifact_keys = [
            (artifact["platform"], artifact["arch"], artifact["format"], artifact["file_name"])
    ...
    def test_verify_rejects_cross_release_or_basename_mismatch_urls(self):
        artifact, fragment = self.make_fragment()
        data = json.loads(fragment.read_text(encoding="utf-8"))
        for bad_url in [
            ...
            metadata, signature = self.sign_fragments([bad_fragment])
            self.assertNotEqual(self.verify_release(metadata, signature, [artifact], check=False).returncode, 0)
    def test_sign_rejects_duplicate_artifact_identity(self):
        _, fragment_one = self.make_fragment(out_name="one.json")
        _, fragment_two = self.make_fragment(out_name="two.json")
    ...
    def test_check_version_accepts_parseable_tags_rejects_mismatch_and_skips_nightly(self):
        ...
        nightly = self.run_script("check-version", "--release-id", "nightly", "--version", "1.4.6")
        self.assertIn("not an official client update source", nightly.stdout)
