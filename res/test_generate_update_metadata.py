...
class GenerateUpdateMetadataTest(unittest.TestCase):
    ...
    def _resign_metadata(self, metadata, signature, metadata_data):
        ...
        private_key = ed25519.Ed25519PrivateKey.from_private_bytes(
            base64.b64decode(self.seed_b64, validate=True)
        )
    ...
    def test_sign_rejects_duplicate_metadata_file_names(self):
        artifact, fragment = self.make_fragment()
        _, duplicate_fragment = self.make_fragment(
    ...
    def test_verify_rejects_duplicate_metadata_file_names(self):
        ...
        result = self.verify_release(metadata, signature, [artifact], check=False)

        self.assertNotEqual(result.returncode, 0)
        self.assertIn("duplicate metadata artifact file_name", result.stderr)
    def test_verify_rejects_duplicate_artifact_selectors(self):
        artifact_one, fragment_one = self.make_fragment(out_name="one.json")
        artifact_two, fragment_two = self.make_fragment(
            artifact_name="rustdesk-1.4.6-aarch64.exe",
            arch="aarch64",
            out_name="two.json",
        )
        metadata, signature, _ = self.run_sign([fragment_one, fragment_two])
        metadata_data = json.loads(metadata.read_text(encoding="utf-8"))
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("duplicate artifact selector", result.stderr)
    def test_sign_rejects_unknown_fragment_field(self):
        _, fragment = self.make_fragment()
        fragment_data = json.loads(fragment.read_text(encoding="utf-8"))
        fragment_data["unexpected"] = "value"
        fragment.write_text(json.dumps(fragment_data), encoding="utf-8")

        _, _, result = self.run_sign([fragment], check=False)

        self.assertNotEqual(result.returncode, 0)
        self.assertIn("unknown fragment field", result.stderr)
    ...
    def test_check_version_rejects_non_ascii_digits(self):
        result = self.run_script(
            "check-version",
            "--release-id",
            "v\u0661.\u0662.\u0663",
    ...
    def test_signed_update_publication_is_serialized_by_tag(self):
        workflow_path = SCRIPT.parents[1] / ".github" / "workflows" / "flutter-build.yml"
        workflow = workflow_path.read_text(encoding="utf-8")
        marker = "  publish-signed-update-metadata:\n"
        self.assertIn(marker, workflow)
        job = workflow.split(marker, 1)[1].split("\n  publish_unsigned:\n", 1)[0]
        self.assertIn("    concurrency:\n", job)
        self.assertIn("      group: signed-update-${{ inputs.upload-tag }}\n", job)
        self.assertIn("      cancel-in-progress: false\n", job)
    def test_signed_update_metadata_is_published_last(self):
        workflow_path = SCRIPT.parents[1] / ".github" / "workflows" / "flutter-build.yml"
        workflow = workflow_path.read_text(encoding="utf-8")
        job = workflow.split("  publish-signed-update-metadata:\n", 1)[1].split("\n  publish_unsigned:\n", 1)[0]
        remove_marker = "      - name: Remove published update metadata\n"
        artifacts_marker = "      - name: Upload signed update artifacts\n"
        signature_marker = "      - name: Upload signed update metadata signature\n"
        metadata_marker = "      - name: Publish signed update metadata\n"
        positions = [job.index(marker) for marker in (remove_marker, artifacts_marker, signature_marker, metadata_marker)]
        self.assertEqual(positions, sorted(positions))
        remove_step = job.split(remove_marker, 1)[1].split(artifacts_marker, 1)[0]
        self.assertIn("          for asset_name in rustdesk-update.json rustdesk-update.json.sig; do\n", remove_step)
        artifacts_step = job.split(artifacts_marker, 1)[1].split(signature_marker, 1)[0]
        self.assertNotIn("            ./rustdesk-update.json\n", artifacts_step)
        self.assertNotIn("            ./rustdesk-update.json.sig\n", artifacts_step)
        signature_step = job.split(signature_marker, 1)[1].split(metadata_marker, 1)[0]
        self.assertIn("          files: ./rustdesk-update.json.sig\n", signature_step)
        metadata_step = job.split(metadata_marker, 1)[1]
        self.assertIn("          files: ./rustdesk-update.json\n", metadata_step)
