...
class FileModel {
  ...
  void onSelectedFiles(dynamic obj) {
    ...
    try {
      ...
      final batchId = int.tryParse(obj['batchId']?.toString() ?? '');
      if (batchId == null) {
        jobController.registerTransferConflictBatch([jobID]);
      } else {
        jobController.registerTransferConflictExternalBatchKey(
            jobID, 'web-selected-files-$batchId');
      }
      webSendLocalFiles(
...
class JobController {
  ...
  final Map<String, int> _transferConflictExternalBatchKeyToBatch = {};
  ...
  void registerTransferConflictBatch(Iterable<int> jobIds) {
    final ids = jobIds.toList(growable: false);
    if (ids.isEmpty) {
      return;
    }
    final batchId = _nextTransferConflictBatchId++;
    for (final jobId in ids) {
      _transferConflictJobToBatch[jobId] = batchId;
    }
  }

  void registerTransferConflictExternalBatchKey(int jobId, String batchKey) {
    final batchId = _transferConflictExternalBatchKeyToBatch.putIfAbsent(
        batchKey, () => _nextTransferConflictBatchId++);
    _transferConflictJobToBatch[jobId] = batchId;
  }
  ...
  void unregisterTransferConflictJob(int jobId) {
    ...
    if (!_transferConflictJobToBatch.containsValue(batchId)) {
      ...
      _transferConflictExternalBatchKeyToBatch.removeWhere(
          (_, currentBatchId) => currentBatchId == batchId);
    }
  }
  ...
  Future<void> cancelTransferConflictBatch(int jobId) async {
    ...
    jobTable.removeWhere((job) => jobIdsToCancel.contains(job.id));
    jobTable.refresh();
    for (final id in batchJobIds) {
      await bind.sessionCancelJob(sessionId: sessionId, actId: id);
    }
  ...
  void clear() {
    ...
    _transferConflictExternalBatchKeyToBatch.clear();
    _transferConflictRememberBatchId = null;
    _transferConflictRememberOverrideConfirm = null;
