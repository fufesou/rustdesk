...
class RelativeMouseModel {
  ...
  Future<bool> _enableNativeRelativeMouseMode() async {
    ...
    try {
      final result = await _hostChannel!.invokeMethod('enableNativeRelativeMouseMode');
      if (result == true) {
  ...
  void onWindowBlur() {
    ...
    // Reset ESC key suppression state when window loses focus.
    // This prevents stale state if user presses ESC, switches window, and releases ESC elsewhere.
    _suppressEscapeKeyUp = false;
  ...
  void _recenterIfNearEdge() {
    ...
    // Threshold: recenter when within 50 points of the edge
  ...
  Future<void> updatePointerLockCenter({Offset? localCenter}) async {
    ...
    try {
      ...
      if (_pointerRegionTopLeftGlobal != null && scale > 0) {
        ...
        if (isMacOS) {
          // macOS: frame is in points, CGWarpMouseCursorPosition expects points
          // We need to account for the title bar height.
          // frame.height is the total window height (including title bar)
          // clientPhysical.height / scale gives us the Flutter view height in points
          ...
          // Windows/Linux: use pixel coordinates with scaling
  ...
  void _applyCursorClipForFrame(Rect frame) {
    ...
    if (_pointerRegionTopLeftGlobal != null && _imageWidgetSize != null && scale > 0) {
      ...
      left = (clientTopLeftScreen.dx + _pointerRegionTopLeftGlobal!.dx * scale).toInt();
      top = (clientTopLeftScreen.dy + _pointerRegionTopLeftGlobal!.dy * scale).toInt();
