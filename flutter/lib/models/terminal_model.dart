...
const _discardSupersededTerminalClipboardCommand =
    'discard_superseded_terminal_clipboard';
...
class TerminalModel with ChangeNotifier {
  ...
  TerminalModel(this.parent, [this.terminalId = 0]) : id = parent.id {
    terminal = RustDeskTerminal(
      ...
      onPendingClipboardWriteDiscarded:
          isWeb ? _discardSupersededWebClipboardWrite : null,
    );
  ...
  void _discardSupersededWebClipboardWrite() {
    try {
      ffiSetByName(_discardSupersededTerminalClipboardCommand);
    } catch (error) {
      debugPrint(
        '[TerminalModel] Failed to discard superseded Web clipboard write: '
        '$error',
      );
    }
  }
